...
func fixInvalidJson(b []byte) ([]byte, bool) {
...
func ExtractObjectIndexes(c string) [][2]int {
	...
	for _, result := range results {
		...
		_, isJson := fixInvalidJson([]byte(retRaw))
...
func ExtractJSONWithRaw(raw string) (results []string, rawStr []string, rerr error) {
	defer func() {
		if err := recover(); err != nil {
			rerr = fmt.Errorf("ExtractJSONWithRaw error: %v", err)
		}
	}()
	...
	for _, obj := range ExtractObjectIndexes(raw) {
		...
		if ret, ok := fixInvalidJson([]byte(jsonStr)); ok {
...
func ExtractStandardJSON(raw string) []string {
	jsonStr, _, _ := ExtractJSONWithRaw(raw)
	return jsonStr
}
